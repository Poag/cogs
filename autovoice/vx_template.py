...
Ported from PhasecoreX's ``pcx_template.py`` unchanged - same library
(Jinja2's ``ImmutableSandboxedEnvironment``), same safety properties
(silently-undefined variables, a hard 0.25s render timeout, and a
deterministic ``random`` filter seeded from the rendering context) so that
custom name/hint templates migrated from AutoRoom keep rendering identically.
"""
