...
from .vx_lib import Perms, SettingDisplay, delete
...
class AutoVoiceCommands(MixinMeta, ABC):
    ...
    @autovoice.command(name="settings", aliases=["about", "info"])
    async def autovoice_settings(self, ctx: commands.Context) -> None:
        """Display current settings."""
        if not ctx.guild:
            return
        autovoice_channel, autovoice_info = await self._get_autovoice_channel_and_info(
            ctx, check_owner=False
        )
        if not autovoice_channel or not autovoice_info:
            return

        room_settings = SettingDisplay("Room Settings")
        owner = ctx.guild.get_member(autovoice_info["owner"])
        if owner:
            room_settings.add(
                "Owner",
                owner.display_name,
        ...
        source_channel = ctx.guild.get_channel(autovoice_info["source_channel"])
        if isinstance(source_channel, discord.VoiceChannel):
            ...
            if member_roles:
                for role in member_roles:
                    autovoice_type = self._get_autovoice_type(autovoice_channel, role)
                    if not access_text:
                        access_text = autovoice_type
                    elif access_text != autovoice_type:
                        # Multiple member roles present, and we can't determine the room type
                        access_text = "custom"
                        break
            else:
                access_text = self._get_autovoice_type(
                    autovoice_channel, autovoice_channel.guild.default_role
                )
        ...
        room_settings.add("Bitrate", f"{autovoice_channel.bitrate // 1000}kbps")
        room_settings.add(
            "Channel Age",
            humanize_timedelta(
                timedelta=datetime.datetime.now(datetime.UTC)
                - autovoice_channel.created_at
            ),
        )
        ...
        denied_members = []
        denied_roles = []
        for member_or_role in autovoice_channel.overwrites:
            if isinstance(member_or_role, discord.Role):
                if self.check_if_member_or_role_allowed(
                    autovoice_channel, member_or_role
                ):
    ...
    @autovoice.command(name="name")
    async def autovoice_name(self, ctx: commands.Context, *, name: str) -> None:
        """Change the name of your AutoVoice room."""
        if not ctx.guild:
            return
        autovoice_channel, autovoice_info = await self._get_autovoice_channel_and_info(
            ctx
        ...
        if name != autovoice_channel.name:
            bucket = self.bucket_autovoice_name.get_bucket(autovoice_channel)
            if bucket:
                retry_after = bucket.update_rate_limit()
                if retry_after:
                    per_display = bucket.per - self.extra_channel_name_change_delay
                    hint_text = error(
                        f"{ctx.message.author.mention}, you can only modify an AutoVoice room name **{bucket.rate}** times "
                ...
                await autovoice_channel.edit(
                    name=name, reason="AutoVoice: User edit room info"
                )
        await ctx.tick()
        await delete(ctx.message, delay=5)

    @autovoice.command(name="bitrate", aliases=["kbps"])
    async def autovoice_bitrate(self, ctx: commands.Context, kbps: int) -> None:
        """Change the bitrate of your AutoVoice room."""
        if not ctx.guild:
            return
        autovoice_channel, autovoice_info = await self._get_autovoice_channel_and_info(
            ctx
        ...
        if bps != autovoice_channel.bitrate:
            await autovoice_channel.edit(
                bitrate=bps, reason="AutoVoice: User edit room info"
            )
        await ctx.tick()
        await delete(ctx.message, delay=5)

    @autovoice.command(name="users", aliases=["userlimit"])
    async def autovoice_users(self, ctx: commands.Context, user_limit: int) -> None:
        """Change the user limit of your AutoVoice room."""
        autovoice_channel, autovoice_info = await self._get_autovoice_channel_and_info(
            ctx
        )
        if not autovoice_channel or not autovoice_info:
        ...
        if limit != autovoice_channel.user_limit:
            await autovoice_channel.edit(
                user_limit=limit, reason="AutoVoice: User edit room info"
            )
        await ctx.tick()
        await delete(ctx.message, delay=5)
    ...
    async def claim(self, ctx: commands.Context) -> None:
        """Claim ownership of this AutoVoice room."""
        if not ctx.guild:
            return
        ...
        if not autovoice_channel or not autovoice_info:
            return
        bucket = self.bucket_autovoice_owner_claim.get_bucket(autovoice_channel)
        old_owner = ctx.guild.get_member(autovoice_info["owner"])
        denied_message = ""
        ...
        if (
            ...
            if old_owner and old_owner in autovoice_channel.members:
                denied_message = (
                    "you can only claim ownership once the AutoVoice room's owner has left"
                )
            elif bucket:
                retry_after = bucket.update_rate_limit()
                if retry_after:
        ...
        if not asc:
            await self._send_temp_error_message(
                ctx,
                "it seems like the AutoVoice Source this room was made from "
                "no longer exists. Because of that, I can no longer modify this room.",
            )
            return
        ...
        if perms.modified:
            await autovoice_channel.edit(
                ...
                reason="AutoVoice: Ownership claimed",
            )
        await self.config.channel(autovoice_channel).owner.set(new_owner.id)
        ...
        if await self._process_allow_deny(ctx, "deny", member_or_role=member_or_role):
            ...
            for member in channel.members:
                if not channel.permissions_for(member).connect:
                    await member.move_to(None, reason="AutoVoice: Deny user")
    ...
    async def _process_allow_deny(
        ...
        *,
        ...
        if not autovoice_channel.permissions_for(
            autovoice_channel.guild.me
        ).manage_roles:
            await self._send_temp_error_message(
                ctx,
        ...
        if not member_or_role:
            ...
            if access != "allow":
                for member in autovoice_channel.members:
            ...
            # - Make sure that the role isn't specifically denied on the source channel
                ...
                if isinstance(member_or_role, discord.Role):
                    ...
                    them_it = "it"
                denied_message = (
                    f"since that {user_role} is not allowed to connect to the AutoVoice Source "
                    f"that this room was made from, I can't allow {them_it} here either."
                )
            ...
            # - Check that the role is equal to or above the lowest allowed (member) role on the source channel
                ...
                denied_message = "this AutoVoice room is using member roles, so I can't allow a lower hierarchy role."
        # Deny a specific user
        # - check that they aren't a mod/admin/owner/room owner/bot itself, then deny user
        # Deny a specific role
        # - Check that it isn't a mod/admin role, then deny role
        elif member_or_role == ctx.guild.me:
            denied_message = "why would I deny myself from entering your room?"
        elif member_or_role == ctx.message.author:
            denied_message = "don't be so hard on yourself! This is your room!"
            ...
            denied_message = f"that's an admin{role_suffix}, so I can't deny them from entering your room."
        elif await self.is_mod_or_mod_role(member_or_role):
            role_suffix = " role" if isinstance(member_or_role, discord.Role) else ""
            denied_message = f"that's a moderator{role_suffix}, so I can't deny them from entering your room."
        ...
        if perms.modified:
            await autovoice_channel.edit(
                overwrites=perms.overwrites or {},
                reason="AutoVoice: Permission change",
            )
        await ctx.tick()
        await delete(ctx.message, delay=5)
        return True
    ...
    async def _get_autovoice_channel_and_info(
        self, ctx: commands.Context, *, check_owner: bool = True
    ) -> tuple[discord.VoiceChannel | None, dict[str, Any] | None]:
        autovoice_channel = self._get_current_voice_channel(ctx.message.author)
        autovoice_info = await self.get_autovoice_info(autovoice_channel)
        if not autovoice_info:
            await self._send_temp_error_message(ctx, "you are not in an AutoVoice room.")
            return None, None
        if check_owner and ctx.message.author.id != autovoice_info["owner"]:
            reason_server = ""
            if not autovoice_info["owner"]:
                reason_server = " (it is a server-managed room)"
            await self._send_temp_error_message(
                ctx, f"you are not the owner of this room{reason_server}."
            )
            return None, None
        return autovoice_channel, autovoice_info

    @staticmethod
    def _get_autovoice_type(autovoice: discord.VoiceChannel, role: discord.Role) -> str:
        """Get the type of access a role has in an AutoVoice room (public, locked, private, etc)."""
        view_channel = role.permissions.view_channel
        connect = role.permissions.connect
        if role in autovoice.overwrites:
            overwrites_allow, overwrites_deny = autovoice.overwrites[role].pair()
